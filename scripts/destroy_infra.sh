...
# Run Terraform to teardown the enviornment
(cd "${ROOT}/terraform"; terraform init -backend-config="bucket=${bucket}" -backend-config="prefix=${prefix}" -input=false)
(cd "${ROOT}/terraform"; terraform destroy -var-file="${common_tfvars_rel_path}" -var-file="${env_specific_tfvars_rel_path}" -input=false -auto-approve)